...
import helper.Utils;
import org.engine.graph.FontTexture;
import org.engine.graph.Material;
import org.engine.graph.Mesh;
import org.joml.Vector3f;

public class TextItem extends GameElement {
    ...
    private Mesh buildMesh() {
        ...
        for(int i=0; i<numChars; i++) {
            ...
            if(i == 0){
                sx = startx;
                ex = startx+charInfo.getWidth();
                sy = 0.0f;
                ey = fontTexture.getHeight();
                sz = ZPOS;
                ez = ZPOS;
            } else {
                ex = startx+charInfo.getWidth();
        ...
        Mesh mesh = new Mesh(posArr, textCoordsArr, normals, indicesArr, bounds);
