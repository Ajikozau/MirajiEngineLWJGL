...
public class HeightMapMesh {
    ...
    private float getHeight(int x, int z, int width, ByteBuffer buffer) {        
        return this.minY + Math.abs(this.maxY - this.minY) * ((float) getRGB(x, z, width, buffer) / (float) MAX_COLOR);
    }
    
    public static int getRGB(int x, int z, int width, ByteBuffer buffer) {       
        return ((0xFF & buffer.get(x * 4 + 3 + z * 4 * width)) << 24) | ((0xFF & buffer.get(x * 4 + 0 + z * 4 * width)) << 16) 
                | ((0xFF & buffer.get(x * 4 + 1 + z * 4 * width)) << 8) | (0xFF & buffer.get(x * 4 + 2 + z * 4 * width));        
    }
