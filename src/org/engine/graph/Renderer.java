...
import org.engine.graph.anim.AnimGameElement;
import org.engine.graph.anim.AnimatedFrame;
...
public class Renderer {
    ...
    private void renderParticles(Window window, Camera camera, Scene scene) {
        ...
        particlesShaderProgram.setUniform("projectionMatrix", window.getProjectionMatrix());
    ...
    private void renderSkyBox(Window window, Camera camera, Scene scene) {
        ...
        if (skyBox != null) {
            ...
            skyBoxShaderProgram.setUniform("projectionMatrix", window.getProjectionMatrix());
            Matrix4f viewMatrix = camera.getViewMatrix();
            ...
            skyBoxShaderProgram.setUniform("modelViewMatrix", transformation.buildModelViewMatrix(skyBox, viewMatrix));
            skyBoxShaderProgram.setUniform("ambientLight", scene.getSceneLight().getSkyBoxLight());
            skyBoxShaderProgram.setUniform("color", mesh.getMaterial().getColor());
    ...
    public void renderScene(Window window, Camera camera, Scene scene) {
        ...
        sceneShaderProgram.setUniform("projectionMatrix", window.getProjectionMatrix());
        ...
        sceneShaderProgram.setUniform("renderShadow", scene.renderShadows() ? 1 : 0);
    ...
    private void renderNonInstancedMeshes(Scene scene, ShaderProgram shader, Matrix4f viewMatrix, Matrix4f lightViewMatrix) {
        ...
        StaticHelpers.iterateSet(mapMeshes.keySet(), mesh -> {
            ...
            mesh.renderList(mapMeshes.get(mesh), (GameElement gameItem) -> {
                ...
                if (gameItem instanceof AnimGameElement) {
                    AnimGameElement animGameItem = (AnimGameElement) gameItem;
    ...
    private void renderInstancedMeshes(Scene scene, ShaderProgram shader, Matrix4f viewMatrix, Matrix4f lightViewMatrix) {
        ...
        // Render each mesh with the associated game Items
        Map<InstancedMesh, List<GameElement>> mapMeshes = scene.getGameInstancedMeshes();
    ...
    private void renderLights(Matrix4f viewMatrix, SceneLight sceneLight) {
        ...
        if(pointLightList != null){
            for (PointLight currPointLight : pointLightList) {
        ...
        if(spotLightList != null){
            for (SpotLight currSpotLight : spotLightList) {
    ...
    private void renderCrossHair(Window window) {
        if (window.getOptions().compatibleProfile) {
            glPushMatrix();
            glLoadIdentity();
            float inc = 0.05f;
            glLineWidth(2.0f);
    ...
    private void renderAxes(Window window, Camera camera) {
        Window.WindowOptions opts = window.getOptions();
